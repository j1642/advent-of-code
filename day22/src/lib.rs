...
// TODO: blocks fall into their final positions
// TODO: determine points contained in a block (ish, not a literal vec of points)
// TODO: determine which bricks touch each other (naive n^2)
...
fn drop_blocks(blocks: &mut Vec<Block>) {
    // blocks fall into their final positions

    // sort by z, deal with lowest blocks first
    // What about blocks aligned on z-axis? - use lower z
    blocks.sort_by(|a, b| 
        min(a.start.z, a.end.z).partial_cmp(min(&b.start.z, &b.end.z)).unwrap());

    for i in 0..blocks.len() {
        let mut block = blocks[i].clone();
        // lowest elavation block starts at z=0
        if i == 0 {
            if blocks[0].start.z > 0 {
                let start_end_diff = blocks[0].end.z - blocks[0].start.z;
                blocks[0].start.z = 0;
                blocks[0].end.z = start_end_diff;
            }
            println!("first: {:?}", block);
            blocks[i] = block;
            continue;
        ...
        if block.start.x != block.end.x {
            for x in block.start.x..=block.end.x {
                top_cubes.push(Point{x: x, y: block.start.y, z: block.start.z});
            }
        } else if block.start.y != block.end.y {
            for y in block.start.y..=block.end.y {
                top_cubes.push(Point{x: block.start.x, y: y, z: block.start.z});
            }
        } else if block.start.z != block.end.z {
            top_cubes.push(Point{x: block.start.x, y: block.start.y, z: block.start.z});
        }
        ...
        for j in (0..i).rev() {
            ...
            if blocks[j].start.x != blocks[j].end.x {
                for x in blocks[j].start.x..=blocks[j].end.x {
                    bottom_cubes.push(Point{x: x, y: blocks[j].start.y, z: blocks[j].start.z});
                }
            } else if blocks[j].start.y != blocks[j].end.y {
                for y in blocks[j].start.y..=blocks[j].end.y {
                    bottom_cubes.push(Point{x: blocks[j].start.x, y: y, z: blocks[j].start.z});
                }
            } else if blocks[j].start.z != blocks[j].end.z {
                bottom_cubes.push(Point{x: blocks[j].start.x, y: blocks[j].start.y, z: blocks[j].start.z});
            }
